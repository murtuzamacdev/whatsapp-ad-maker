...
const Template4 = ({ productData }) => {
    ...
    return (<div style={{ backgroundImage: "url(" + productData.productImage + ")" }} className="temp4-ctnr">

        {/* Shapes */}
        <div className="price-ctrn" style={{ background: 'linear-gradient(233.12deg, ' + productData.selectedBackgroundColor + ' 33.2%, ' + secondaryBgForGradient + ' 75.65%)' }}> </div>
        <div className='content-ctrn' style={{ background: 'linear-gradient(233.12deg, ' + productData.selectedBackgroundColor + ' 33.2%, ' + secondaryBgForGradient + ' 75.65%)' }}></div>
        ...
        <div className="mt-3 d-flex justify-content-center single-line flex-column top-part align-items-center price-order">
            ...
            {productData.productPrice !== '' && <div className="product-price pt-2 pb-2 pl-4 pr-4"><small>{currencies.find((item) => item.code === productData.currencyCode).symbol}</small> {productData.productPrice}<small>.00</small></div>}
        </div>

        <div className="bottom-ctrn">

            <div className="purple-ctnr pt-4 pb-4 pl-3 pr-3">
                <p className="product-name p-2 m-0">{productData.productName}</p>
